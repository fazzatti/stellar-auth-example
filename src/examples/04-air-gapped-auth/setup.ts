...
=============================================
Setup of Example 04 - Air Gapped Auth Demo
=============================================
`,
    "blue"
  )
...
await saveToJsonFile<ProxyConfig>(proxyConfig, io.proxyConfigFileName);
console.log("Setup complete!");

console.log("Generating airgapped config...");
const airGappedConfig: AirGappedConfig = {
  secureUserSk: userKeys.secret(),
  allowedContracts: [contractId],
};

await saveToJsonFile<AirGappedConfig>(
  airGappedConfig,
  io.airGappedConfigFileName
);

console.log("Generating sample input files...");

const proxyInput: ProxyInput = {
  contractId,
  function: "swap",
  functionArgs: {
    userPk: userKeys.publicKey(),
    isSellAssetA: true,
    amount: "100000000",
  },
...
await saveToJsonFile<ProxyInput>(proxyInput, io.proxyInputFileName);

const airGappedInput: AirGappedSignInput = {
  contractId,
  function: "swap",
  functionArgs: {
    userPk: userKeys.publicKey(),
